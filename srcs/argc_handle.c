...
int handle_argc(t_dining *dining, int argc, char **argv)
{
    ...
    if (!ft_atoi(argv[1], &(dining->info.num_of_philo)) || !ft_atoi(argv[2], &(dining->info.time_to_die)) || !ft_atoi(argv[3], &(dining->info.time_to_eat)) || !ft_atoi(argv[4], &(dining->info.time_to_sleep)) || !check_arg(argc, argv, &(dining->info.num_must_eat)))
        return (print_err("Error\n"));
    dining->info.eat_finish = 0;
    dining->info.finish = 0;
    dining->info.retERR = 0;
